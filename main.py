...
def calculate_paths(gri_graph, rou_lists):
    ...
    for rou in rou_lists.keys():
        ...
        for m in range(len(rou_lists[rou]) - 1):
            ...
            if not st.is_settled() or not nb.is_settled():
                source_candidates, target_candidates = get_candidates(st, nb, st_node, nb_node)
...
def get_candidates(st, neighbor, st_node, nb_node):
    source_candidates = [st_node]
    target_candidates = [nb_node]
...
def geodesic_dists(skelet_graph):
    geo_dists = {}
    i = 0
...
if __name__ == "__main__":
    ...
    grid_graph = calculate_paths(grid_graph, route_lists)
    ...
    geo_dists = geodesic_dists(skel_graph)
    for st in [st for st in stops.keys() if grid_graph.nodes[st]['drawn']]:
